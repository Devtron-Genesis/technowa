...
function hasta_image_style($variables) {
  $variables['alt'] = empty($variables['alt']) ? ' ' : '';
 ...
 * Overrides theme_menu_tree().
 */
function hasta_menu_tree__main_menu(&$variables) {
...
function hasta_menu_link(array $variables) {
  ...
  if ($element['#below']) {
    ...
    $sub_menu = '<ul class = "nav-child dropdown-menu">' . drupal_render($element['#below']) . '</ul>';
  }
...
function hasta_url_outbound_alter(&$path, &$options, $original_path) {
